...
aptver19=$("${R_HOME}/bin/Rscript" -e 'cat(ifelse(as.package_version(readLines(pipe("pkg-config --modversion apt-pkg"))) >= as.package_version("1.9.0"), "-DAPT_Version2 -DAPT_COMPILING_APT", ""))')
# Same for 2.5.0
aptver25=$("${R_HOME}/bin/Rscript" -e 'cat(ifelse(as.package_version(readLines(pipe("pkg-config --modversion apt-pkg"))) >= as.package_version("2.5.0"), "-DAPT_Version_2_5", ""))')


sed -e 's/@compilearg@/-DRcppAPT_Good_System/' \
    -e "s/@apt19versionarg@/${aptver19}/" \
    -e "s/@apt25versionarg@/${aptver25}/" \
